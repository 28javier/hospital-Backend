...
const getUsuarios = async (req = request, res = response) => {
  ...
  try {
    ...
    //                             .skip(desde).limit(5);
    // const totalUsuario = await UsuarioDB.count();
    const [usuarios, totalUsuario] = await Promise.all([
      UsuarioDB.find({},"nombre1 nombre2 apellido1 apellido2 email rol img google")
        .skip(desde)
        .limit(5),
...
const updateUsuarios = async (req = request, res = response) => {
  ...
  try {
    ...
    campos.email = email;
    const usuarioActualizado = await UsuarioDB.findByIdAndUpdate(uid, campos, {
      new: true,
    });
    res.json({
      ok: true,
